...
/// CRTP helper for declaring equality comparison. Defines operator== and
/// operator!=
template <typename T> class EqualityComparable {
public:
  ~EqualityComparable() {
    static_assert(
        std::is_same<decltype(std::declval<const T>().Equals(
                         std::declval<const T>())),
                     bool>::value,
...
template <typename T> class ToStringOstreamable {
public:
  ~ToStringOstreamable() {
    static_assert(
        std::is_same<decltype(std::declval<const T>().ToString()),
                     std::string>::value,
...
template <typename... Args> std::string StringBuilder(Args &&...args) {
...
public:
  ...
  bool operator==(const StatusDetail &other) const noexcept {
    return std::string(type_id()) == other.type_id() &&
...
/// attached.
class Status : public util::EqualityComparable<Status>,
...
public:
  ...
  /// \brief Pluggable constructor for use by sub-systems.  detail cannot be
  /// null.
  ...
  template <typename... Args> static Status FromCode(StatusCode code) {
    return Status(code, "");
  }
  ...
  template <typename... Args> static Status OutOfMemory(Args &&...args) {
    return Status::FromArgs(StatusCode::OutOfMemory,
                            std::forward<Args>(args)...);
  }

  /// Return an error status for failed key lookups (e.g. column name in a
  /// table)
  template <typename... Args> static Status KeyError(Args &&...args) {
  ...
  template <typename... Args> static Status TypeError(Args &&...args) {
    return Status::FromArgs(StatusCode::TypeError, std::forward<Args>(args)...);
  }

  /// Return an error status for unknown errors
  template <typename... Args> static Status UnknownError(Args &&...args) {
    return Status::FromArgs(StatusCode::UnknownError,
                            std::forward<Args>(args)...);
  }

  template <typename... Args> static Status Uninitialized() {
    return Status::FromCode(StatusCode::Uninitialized);
  }

  /// Return an error status when an operation or a combination of operation and
  /// data types is unimplemented
  template <typename... Args> static Status NotImplemented(Args &&...args) {
  ...
  /// parsing)
  template <typename... Args> static Status Invalid(Args &&...args) {
    return Status::FromArgs(StatusCode::Invalid, std::forward<Args>(args)...);
  }

  /// Return an error status for cancelled operation
  template <typename... Args> static Status Cancelled(Args &&...args) {
    return Status::FromArgs(StatusCode::Cancelled, std::forward<Args>(args)...);
  }

  /// Return an error status when an index is out of bounds
  template <typename... Args> static Status IndexError(Args &&...args) {
    return Status::FromArgs(StatusCode::IndexError,
                            std::forward<Args>(args)...);
  }

  /// Return an error status when a container's capacity would exceed its limits
  template <typename... Args> static Status CapacityError(Args &&...args) {
    return Status::FromArgs(StatusCode::CapacityError,
                            std::forward<Args>(args)...);
  }

  /// Return an error status when some IO-related operation failed
  template <typename... Args> static Status IOError(Args &&...args) {
  ...
  template <typename... Args> static Status SerializationError(Args &&...args) {
    return Status::FromArgs(StatusCode::SerializationError,
                            std::forward<Args>(args)...);
  }

  template <typename... Args> static Status RError(Args &&...args) {
    return Status::FromArgs(StatusCode::RError, std::forward<Args>(args)...);
  }

  template <typename... Args> static Status CodeGenError(Args &&...args) {
    return Status::FromArgs(StatusCode::CodeGenError,
                            std::forward<Args>(args)...);
  }
  ...
  template <typename... Args> static Status ExecutionError(Args &&...args) {
    return Status::FromArgs(StatusCode::ExecutionError,
                            std::forward<Args>(args)...);
  }

  template <typename... Args> static Status AlreadyExists(Args &&...args) {
    return Status::FromArgs(StatusCode::AlreadyExists,
                            std::forward<Args>(args)...);
  }
  ...
  template <typename... Args> Status WithMessage(Args &&...args) const {
