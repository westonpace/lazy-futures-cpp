...
template <typename> struct EnsureResult;
...
template <typename T> class AlignedStorage {
public:
  ...
  template <typename... A> void construct(A &&...args) noexcept {
    new (&data_) T(std::forward<A>(args)...);
  }

  template <typename V> void assign(V &&v) noexcept {
    *get() = std::forward<V>(v);
  }
  ...
  void move_assign(AlignedStorage *other) noexcept {
    *get() = std::move(*other->get());
  }

  template <bool CanMemcpy = can_memcpy>
  static typename std::enable_if<CanMemcpy>::type
  move_construct_several(AlignedStorage *ARROW_RESTRICT src,
...
/// ```
template <class T> class Result : public util::EqualityComparable<Result<T>> {
...
public:
  ...
  /// Helper method for implementing Status returning functions in terms of
  /// semantically equivalent Result returning functions. For example:
  ///
  ...
  /// Move and return the internally stored value or alternative if an error is
  /// stored.
  ...
  template <typename G> T ValueOrElse(G &&generate_alternative) && {
    if (ok()) {
      return MoveValueUnsafe();
    }
  ...
  /// Apply a function to the internally stored value to produce a new result or
  /// propagate the stored error.
  template <typename M>
  typename EnsureResult<
      decltype(std::declval<M &&>()(std::declval<T &&>()))>::type
  Map(M &&m) && {
    if (!ok()) {
      return status();
    }
  ...
  template <typename M>
  typename EnsureResult<
      decltype(std::declval<M &&>()(std::declval<const T &>()))>::type
  Map(M &&m) const & {
    if (!ok()) {
      return status();
    }
  ...
  /// Cast the internally stored value to produce a new result or propagate the
  /// stored error.
  template <typename U, typename E = typename std::enable_if<
                            std::is_constructible<U, T>::value>::type>
  Result<U> As() && {
...
private:
  ...
  template <typename U> void ConstructValue(U &&u) noexcept {
...
template <typename T> inline Status GenericToStatus(Result<T> &&res) {
...
template <typename T> struct EnsureResult { using type = Result<T>; };
