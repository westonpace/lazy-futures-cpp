...
public:
  ...
  template <typename F> static std::true_type is_overloaded_impl(...);
  ...
  static typename std::tuple_element<I, std::tuple<A...>>::type
      argument_type_impl(R (F::*)(A...));

  template <std::size_t I, typename F, typename R, typename... A>
  static typename std::tuple_element<I, std::tuple<A...>>::type
  argument_type_impl(R (F::*)(A...) const);
  ...
  template <typename F, typename R, typename... A>
  static std::integral_constant<int, sizeof...(A)>
      argument_count_impl(R (F::*)(A...));

  template <typename F, typename R, typename... A>
  static std::integral_constant<int, sizeof...(A)>
  argument_count_impl(R (F::*)(A...) const);
  ...
  /// possible signature. Will be true_type for objects which define multiple
  /// operator() or which define a template operator()
  template <typename F>
  using is_overloaded =
      decltype(is_overloaded_impl<typename std::decay<F>::type>(nullptr));

  template <typename F, typename T = void>
  using enable_if_overloaded =
...
/// signature. Invoking it results in destruction of the lambda, freeing any
/// state/references immediately. Invoking a default constructed FnOnce or one
/// which has already been invoked will segfault.
template <typename Signature> class FnOnce;

template <typename R, typename... A> class FnOnce<R(A...)> {
public:
  FnOnce() = default;

  template <typename Fn,
            typename = typename std::enable_if<std::is_convertible<
                decltype(std::declval<Fn &&>()(std::declval<A>()...)),
                R>::value>::type>
  FnOnce(Fn fn)
      : impl_(new FnImpl<Fn>(std::move(fn))) { // NOLINT runtime/explicit
  }
...
private:
  ...
  template <typename Fn> struct FnImpl : Impl {
...
template <typename> struct EnsureFuture;

template <typename T = internal::Empty> class Future;

template <typename T = internal::Empty> class WeakFuture;
...
struct result_of<
    Fn(A...), std::void_t<decltype(std::declval<Fn>()(std::declval<A>()...))>> {
...
template <typename T> struct SyncType { using type = Result<T>; };

template <> struct SyncType<internal::Empty> { using type = Status; };

template <typename Fn>
using first_arg_is_status = std::is_same<
    typename std::decay<internal::call_traits::argument_type<0, Fn>>::type,
    Status>;
...
using if_has_no_args =
    typename std::conditional<Count::value == 0, Then, Else>::type;
...
struct ContinueFuture {
  template <typename Return> struct ForReturnImpl;
  ...
  // OK.
  template <
      typename ContinueFunc, typename... Args,
  ...
  /// and mark the future finished with the callback result.  It will get
  /// promoted to Result<T> as part of MarkFinished if it isn't already.
  ///
  /// If the callback returns Status and we return Future<> then also send the
  /// callback result as-is to the destination future.
  template <
      typename ContinueFunc, typename... Args,
      typename ContinueResult = result_of_t<ContinueFunc && (Args && ...)>,
      typename NextFuture = ForReturn<ContinueResult>>
  ...
  /// If the callback returns a Result and the next future is Future<> then we
  /// mark the future finished with the callback result.
  ///
  /// It may seem odd that the next future is Future<> when the callback returns
  /// a result but this can occur if the OnFailure callback returns a result
  /// while the OnSuccess callback is void/Status (e.g. you would get this
  /// calling the one-arg version of Then with an OnSuccess callback that
  /// returns void)
  template <
      typename ContinueFunc, typename... Args,
      typename ContinueResult = result_of_t<ContinueFunc && (Args && ...)>,
  ...
  operator()(NextFuture next, ContinueFunc &&f, Args &&...a) const {
    next.MarkFinished(
        std::forward<ContinueFunc>(f)(std::forward<Args>(a)...).status());
  }

  /// If the callback returns a Future<T> then we return Future<T>.  We create a
  /// new future and add a callback to the future given to us by the user that
  /// forwards the result to the future we just created
  template <
      typename ContinueFunc, typename... Args,
      typename ContinueResult = result_of_t<ContinueFunc && (Args && ...)>,
      typename NextFuture = ForReturn<ContinueResult>>
  typename std::enable_if<is_future<ContinueResult>::value>::type
  operator()(NextFuture next, ContinueFunc &&f, Args &&...a) const {
...
/// Helper struct which tells us what kind of Future gets returned from `Then`
/// based on the return type of the OnSuccess callback
template <> struct ContinueFuture::ForReturnImpl<void> {
  using type = Future<>;
};

template <> struct ContinueFuture::ForReturnImpl<Status> {
  using type = Future<>;
};

template <typename R> struct ContinueFuture::ForReturnImpl {
  using type = Future<R>;
};

template <typename T> struct ContinueFuture::ForReturnImpl<Result<T>> {
  using type = Future<T>;
};

template <typename T> struct ContinueFuture::ForReturnImpl<Future<T>> {
  using type = Future<T>;
};
...
inline bool IsFutureFinished(FutureState state) {
  return state != FutureState::PENDING;
}

/// \brief Describe whether the callback should be scheduled or run
/// synchronously
...
public:
  ...
  template <typename ValueType> Result<ValueType> *CastResult() const {
...
public:
  ...
  /// Status of the original. Marking the returned Future Finished is not
  /// supported.
  ...
  ///
  /// `true` is returned if the Future completed, `false` if the timeout
  /// expired. Note a `false` value is only indicative, as the Future can
  ...
  /// future with this method you must ensure that future is eventually
  /// completed (with success or failure).  Creating a future, returning it, and
  /// never completing the future can lead to memory leaks (for example, see
  /// Loop).
  static Future Make() {
    Future fut;
  ...
  struct WrapResultyOnComplete {
    template <typename OnComplete> struct Callback {
  ...
  struct WrapStatusyOnComplete {
    template <typename OnComplete> struct Callback {
      static_assert(
          std::is_same<internal::Empty, ValueType>::value,
  ...
  ///
  /// WARNING: callbacks may hold arbitrary references, including cyclic
  /// references. Since callbacks will only be destroyed after they are invoked,
  /// this can lead to memory leaks if a Future is never marked finished
  /// (abandoned):
  ///
  /// {
  ...
  /// holds a cyclic reference to itself through the callback.
  ...
  void AddCallback(OnComplete on_complete) const {
    ...
    // least one thread will be waiting for MarkFinished to return. Thus it's
    // safe to keep a weak reference to impl_ here
    impl_->AddCallback(Callback{std::move(on_complete)});
  }

  /// \brief Overload of AddCallback that will return false instead of running
  /// synchronously
  ///
  /// This overload will guarantee the callback is never run synchronously.  If
  /// the future is already finished then it will simply return false.  This can
  /// be useful to avoid stack overflow in a situation where you have recursive
  /// Futures.  For an example see the Loop function
  ///
  /// Takes in a callback factory function to allow moving callbacks (the
  /// factory function will only be called if the callback can successfully be
  /// added)
  ///
  /// Returns true if a callback was actually added and false if the callback
  /// failed to add because the future was marked complete.
  ...
  bool TryAddCallback(const CallbackFactory &callback_factory) const {
    return impl_->TryAddCallback(
        [&]() { return Callback{callback_factory()}; });
  }

  template <typename OnSuccess, typename OnFailure> struct ThenOnComplete {
    static constexpr bool has_no_args =
        internal::call_traits::argument_count<OnSuccess>::value == 0;

    using ContinuedFuture =
        detail::ContinueFuture::ForSignature<detail::if_has_no_args<
            OnSuccess, OnSuccess && (), OnSuccess && (const T &)>>;

    static_assert(
        std::is_same<
            detail::ContinueFuture::ForSignature<OnFailure && (const Status &)>,
    ...
    using OnSuccessArg =
        typename std::decay<internal::call_traits::argument_type<
            0, detail::if_has_no_args<OnSuccess, DummyOnSuccess, OnSuccess>>>::
            type;

    static_assert(
        !std::is_same<OnSuccessArg,
                      typename EnsureResult<OnSuccessArg>::type>::value,
        "OnSuccess' argument should not be a Result");

    void operator()(const Result<T> &result) && {
      detail::ContinueFuture continue_future;
      if (ARROW_PREDICT_TRUE(result.ok())) {
        // move on_failure to a(n immediately destroyed) temporary to free its
        // resources
        OnFailure(std::move(on_failure));
        continue_future.IgnoringArgsIf(
            detail::if_has_no_args<OnSuccess, std::true_type,
                                   std::false_type>{},
            std::move(next), std::move(on_success), result.ValueOrDie());
      } else {
        OnSuccess(std::move(on_success));
        continue_future(std::move(next), std::move(on_failure),
                        result.status());
      }
    }
  ...
  template <typename OnSuccess> struct PassthruOnFailure {
    using ContinuedFuture =
        detail::ContinueFuture::ForSignature<detail::if_has_no_args<
            OnSuccess, OnSuccess && (), OnSuccess && (const T &)>>;
  ...
  /// (whatever callback is registered with this function will run before
  /// MarkFinished returns). Avoid long-running callbacks in favor of submitting
  /// a task to an Executor and returning the future.
  ///
  /// Two callbacks are supported:
  /// - OnSuccess, called with the result (const ValueType&) on successul
  /// completion.
  ///              for an empty future this will be called with nothing ()
  /// - OnFailure, called with the error (const Status&) on failed completion.
  ///              This callback is optional and defaults to a passthru of any
  ///              errors.
  ///
  /// Then() returns a Future whose ValueType is derived from the return type of
  /// the callbacks. If a callback returns:
  /// - void, a Future<> will be returned which will completes successully as
  /// soon
  ///   as the callback runs.
  /// - Status, a Future<> will be returned which will complete with the
  /// returned Status
  ///   as soon as the callback runs.
  /// - V or Result<V>, a Future<V> will be returned which will complete with
  /// the result
  ///   of invoking the callback as soon as the callback runs.
  /// - Future<V>, a Future<V> will be returned which will be marked complete
  /// when the
  ...
  /// successully complete even if this Future fails.
  ///
  /// If this future is already completed then the callback will be run
  /// immediately and the returned future may already be marked complete.
  ///
  /// See AddCallback for general considerations when writing callbacks.
  template <typename OnSuccess,
            typename OnFailure = PassthruOnFailure<OnSuccess>,
...
protected:
  ...
  Result<ValueType> *GetResult() const {
    return impl_->CastResult<ValueType>();
  }

  void SetResult(Result<ValueType> res) {
    impl_->result_ = {new Result<ValueType>(std::move(res)), [](void *p) {
                        delete static_cast<Result<ValueType> *>(p);
                      }};
  }
...
template <>
inline typename Future<internal::Empty>::SyncType
FutureToSync<internal::Empty>(const Future<internal::Empty> &fut) {
  return fut.status();
}
...
/// If a Result<Future> holds an error instead of a Future, construct a finished
/// Future holding that error.
template <typename T>
...
Future<std::vector<Result<T>>> All(std::vector<Future<T>> futures) {
  ...
  for (const Future<T> &future : state->futures) {
    future.AddCallback([state, out](const Result<T> &) mutable {
      if (state->n_remaining.fetch_sub(1) != 1)
        return;
...
struct Continue {
  template <typename T> operator std::optional<T>() && { // NOLINT explicit
    return {};
  }
};
...
template <typename T = internal::Empty> using ControlFlow = std::optional<T>;

/// \brief Loop through an asynchronous sequence
///
/// \param[in] iterate A generator of Future<ControlFlow<BreakValue>>. On
/// completion of each yielded future the resulting ControlFlow will be
/// examined. A Break will terminate the loop, while a Continue will re-invoke
/// `iterate`.
///
...
Future<BreakValueType> Loop(Iterate iterate) {
  struct Callback {
    ...
    void operator()(const Result<Control> &maybe_control) && {
      if (CheckForTermination(maybe_control))
        return;
      ...
      while (true) {
        ...
        if (CheckForTermination(control_fut.result()))
          return;
    ...
    // If the future returned by control_fut is never completed then we will be
    // hanging on to break_fut forever even if the listener has given up
    // listening on it.  Instead we rely on the fact that a producer (the caller
    // of Future<>::Make) is always responsible for completing the futures they
    // create.
...
template <typename T> Future<T> ToFuture(T value) {
  return Future<T>::MakeFinished(std::move(value));
}

template <typename T> Future<T> ToFuture(Result<T> maybe_value) {
  return Future<T>::MakeFinished(std::move(maybe_value));
}

template <typename T> Future<T> ToFuture(Future<T> fut) {
  return std::move(fut);
}

template <typename T> struct EnsureFuture {
  using type = decltype(ToFuture(std::declval<T>()));
};

using Task = internal::FnOnce<void()>;
class Executor {
public:
...
// template <typename R, typename... A> using FuncType = std::function<R, A...>;
template <typename T> using Supplier = FuncType<Result<T>()>;
